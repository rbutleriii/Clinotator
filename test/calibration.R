...
P_boot <- one.boot(P_dist$CTRS, mean, R=(P_dist_n + 1), tr=.20)
...
LP_boot <- one.boot(LP_dist$CTRS, mean, R=(LP_dist_n + 1), tr=.20)
...
# US Set
US_dist <- select(filter(big_tbl, grepl('Pathogenic($|,)', CVCS) & CVNA >= 2),c(CTRS))
US_dist_n <- length(US_dist$CTRS)
ggplot(US_dist, aes(CTRS)) + geom_density()
US_boot <- one.boot(US_dist$CTRS, mean, R=(US_dist_n + 1), tr=.20)
...
LB_boot <- one.boot(LB_dist$CTRS, mean, R=(LB_dist_n + 1), tr=.20)
...
B_boot <- one.boot(B_dist$CTRS, mean, R=(B_dist_n + 1), tr=.20)
